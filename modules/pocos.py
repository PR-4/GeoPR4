...
class PR4:
    def pocosRecebidos():
        # Importando dados
        recebido = pd.read_excel(
            "../inputs/pocos/pocos_recebidos_updated.xlsx",
        ...
        recebido = gpd.GeoDataFrame(
            recebido,
            geometry=gpd.points_from_xy(
                recebido.LATITUDE_BASE_DD, recebido.LONGITUDE_BASE_DD
            ),
        )
        ...
        for _, row in recebido.iterrows():
            ...
            popup_content += (
                f"<b>Profundidade:</b> {row['PROFUNDIDADE_SONDADOR_M']}<br>"
            )
            ...
            if pd.notnull(row["AGP"]):
                popup_content += (
                    f"<b>AGP:</b> {'Sim' if row['AGP'] == 'Existe' else 'Não'}<br>"
                )
            ...
            marker = folium.Circle(
                [row["LATITUDE_BASE_DD"], row["LONGITUDE_BASE_DD"]],
                popup=folium.Popup(popup_content, max_width=1000, lazy=True),
                color="crimson",
                radius=1000,
                fill=True,
            )
        ...
        pocos_group = folium.FeatureGroup(name="Pocos Recebidos", show=True)
    ...
    def pocosSelGePreSal():
        # Importando dados
        selecaoGePreSal = pd.read_excel(
            "../inputs/pocos/pocos_recebidos_e_selecao.xlsx",
        ...
        selecaoGePreSal = gpd.GeoDataFrame(
            selecaoGePreSal,
            geometry=gpd.points_from_xy(
                selecaoGePreSal.LATITUDE_BASE_DD, selecaoGePreSal.LONGITUDE_BASE_DD
            ),
        )
        ...
        # Criar grupo para os marcadores
        pocos_group = folium.FeatureGroup(
            name="Pocos Seleção\nGeradora PreSal", show=True
        )
        for marker in markers:
            marker.add_to(pocos_group)
    ...
    def pocosPriPedPetANP():
        # Importando dados
        primeiroPedPetANP = pd.read_excel(
            "../inputs/pocos/pocos_recebidos_e_selecao.xlsx",
        ...
        primeiroPedPetANP = gpd.GeoDataFrame(
            primeiroPedPetANP,
            geometry=gpd.points_from_xy(
                primeiroPedPetANP.LATITUDE_BASE_DD, primeiroPedPetANP.LONGITUDE_BASE_DD
            ),
        )
    ...
    def pocosUFF():
        # Importando dados
        pocosNaUFF = pd.read_excel(
            "../inputs/pocos/presal_uff.xlsx",
            sheet_name="POCO_UFF",
            header=0,
            decimal=",",
            engine="openpyxl",
        ...
        pocosNaUFF = gpd.GeoDataFrame(
            pocosNaUFF,
            geometry=gpd.points_from_xy(
                pocosNaUFF.LATITUDE_BASE_DD, pocosNaUFF.LONGITUDE_BASE_DD
            ),
        )
        ...
        # Criar grupo para os marcadores
        pocos_group = folium.FeatureGroup(name="Pocos UFF", show=True)
        for marker in markers:
            marker.add_to(pocos_group)
