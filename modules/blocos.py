...
class publico:
    ...
    def outerhigh():
        outerhigh_dbf = geopandas.read_file('../inputs/shapes/Santos_outerhigh/santos_outer_high.dbf')
        ...
        outerhigh = folium.GeoJson(data=outerhigh_dbf["geometry"],
                                   style_function=lambda feature: {
                                   'fillColor': 'green',
                                   'color': 'black',     #border color for the color fills
                                   'weight': 1,          #how thick the border has to be
    ...
    def square_selection():
        square_dbf = geopandas.read_file('../inputs/shapes/Santos_outerhigh/square_selection/santos_external_high_squareShape.dbf')
        ...
        square = folium.GeoJson(data=outerhigh_dbf["geometry"],
                                style_function=lambda feature: {
                                'fillColor': 'gray',
                                'color': 'black',     #border color for the color fills
                                'weight': 1,          #how thick the border has to be
